...
const Calculator = () => {
    ...
    const newValue = useRef(false);

    const handleChangeMode = useCallback((modo) => {
        if(secondScore.current.textContent !== '' && Number(score.current.textContent) !== 0) {
            returnValue();
            mode.current = modo;
            secondScore.current.textContent = score.current.textContent;
        ...
        currentMode.current.textContent = mode.current === 0 ? '' : modes[mode.current-1];
    }, []);
    ...
    const returnValue = () => {
        if(mode.current !== 0) {
            ...
            if(mode.current === 1) {
                score.current.textContent = Number(secondScore.current.textContent)+Number(score.current.textContent);
    ...
    const setReverse = () => {
        score.current.textContent = -Number(score.current.textContent);
    }
    ...
    const handleDelButton = () => {
        ...
        if(Number(score.current.textContent) !== 0) {
            if(score.current.textContent.length === 1 || (score.current.textContent.length === 2 && score.current.textContent.indexOf('-') > -1)) {
                score.current.textContent = 0;
            } else
                score.current.textContent = score.current.textContent.slice(0, score.current.textContent.length-1);
        ...
        <div className="calculator">
            ...
            <div>
                {modes.map((modo, index) => (
                    <button key={index} onClick={() => handleChangeMode(index+1)}>{modo}</button>
                ))}
            </div>
            <div className="boxNumsReturn">
                <div className="nums">
                    {nums.map((num, i) => (
                        <button key={i} onClick={setScore}>{num}</button>
                    ))}
